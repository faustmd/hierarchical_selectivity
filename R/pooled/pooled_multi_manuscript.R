...
# Pull in pooled catch data for multifilament gear
data_short <- read.csv("data/multi_catch_pooled.csv")
...
data_pooled <- list(
  mesh = meshes,
  lens = data_short$length_class,
  catches = as.matrix(data_short[, -c(1:2)])
)
colnames(data_pooled$catches) <- NULL
data_pooled$rel_size <- data_pooled$mesh / data_pooled$mesh[1]

# Set-specific multifilament catch data to use with pooled parameter estimates
# this is used in model selection process
data_long <- read.csv("data/multi_catch_by_net.csv")

# remove this net due to wrong coordinates
data_long <- data_long[data_long$net_id != 2012005, ]

# Create list for TMB
data_by_net <- list(
  mesh = meshes,
  lens = data_long$length_class,
...
# version of the multifilament catch data
...
# Just calculate log-likelihood using pooled parameters
obj <- MakeADFun(f_gamma, pars)
obj$report()$jnll
